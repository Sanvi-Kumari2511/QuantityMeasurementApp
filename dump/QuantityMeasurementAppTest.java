...
// Volume Enums Test
	...
	void testConversion_LitreToMillilitre() {
		Quantity<VolumeUnit> v = new Quantity<>(1.0, VolumeUnit.LITRE).convertTo(VolumeUnit.MILLILITRE);
	...
	void testConvert_MillilitreToGallon() {
		Quantity<VolumeUnit> v = new Quantity<>(1000.0, VolumeUnit.MILLILITRE).convertTo(VolumeUnit.GALLON);
	...
	void testConvert_SameUnit_NoChange() {
		Quantity<VolumeUnit> v = new Quantity<>(5.0, VolumeUnit.LITRE).convertTo(VolumeUnit.LITRE);
...
// UC-12 subtraction and division
	...
	void testSubtraction_AllMeasurementCategories() {
		// Length
		assertEquals(5.0,
				new Quantity<>(10.0, LengthUnit.FEET).subtract(new Quantity<>(5.0, LengthUnit.FEET)).getValue(),
				0.0001);

		// Weight
		assertEquals(5.0,
				new Quantity<>(10.0, WeightUnit.KILOGRAM).subtract(new Quantity<>(5.0, WeightUnit.KILOGRAM)).getValue(),
				0.0001);

		// Volume
		assertEquals(3.0,
				new Quantity<>(5.0, VolumeUnit.LITRE).subtract(new Quantity<>(2.0, VolumeUnit.LITRE)).getValue(),
				0.0001);
	}
	...
	void testDivision_NonCommutative() {
		double ab = new Quantity<>(10.0, LengthUnit.FEET).divide(new Quantity<>(5.0, LengthUnit.FEET));
		double ba = new Quantity<>(5.0, LengthUnit.FEET).divide(new Quantity<>(10.0, LengthUnit.FEET));
