...
public class QuantityMeasurementAppTest {
	...
	void testMultipleFeetComparison() {
		assertTrue(new Length(3.0, LengthUnit.FEET).equals(new Length(36.0, LengthUnit.INCHES)));
	}
	...
	void yardNotEqualToInches() {
		assertFalse(new Length(1.0, LengthUnit.YARDS).equals(new Length(10.0, LengthUnit.INCHES)));
	}
	...
	void differentValuesSameUnitNotEqual() {
		assertFalse(new Length(2.0, LengthUnit.FEET).equals(new Length(3.0, LengthUnit.FEET)));
	}
	...
	void convertFeetToInches() {
		assertEquals(12.0, QuantityMeasurementApp.demonstrateLengthConversion(1.0, LengthUnit.FEET, LengthUnit.INCHES),
				EPS);
	}
	...
	void convertYardsToInchesUsingOverloadedMethod() {
		...
		Length inches = QuantityMeasurementApp.demonstrateLengthConversion(yards, LengthUnit.INCHES);
	...
	void convertZeroFeetToInches() {
		assertEquals(0.0, QuantityMeasurementApp.demonstrateLengthConversion(0.0, LengthUnit.FEET, LengthUnit.INCHES),
				EPS);
	}
	...
	public void addFeetAndInchesWithTargetUnitInches() {
		...
		Length sum = QuantityMeasurementApp.demonstrateLengthAddition(length1, length2, Length.LengthUnit.INCHES);
